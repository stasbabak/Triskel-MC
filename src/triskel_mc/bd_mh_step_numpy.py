...
def redblue_mask_np(
    rng: np.random.Generator, C: int, W: int
...
def propose_stretch_redblue_slot_np(
    ...
    a: float = 2.0,
    partner_pool_mask: Optional[
        np.ndarray
    ] = None,  # if provided, partners restricted to this pool
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
...
def ps_swap_pass_inplace(
    ...
    for p in range(idx_low.shape[0]):
        ...
        j = idx_high[p]
        mask_w = (
            accept_pairs[i] | accept_pairs[j]
        )  # (W,) booleans for walkers at this pair
        if not mask_w.any():
            continue
        # swap (φ, m, rest, logπ) at (i,<mask_w>) <-> (j,<mask_w>)
        mw = mask_w
        ps_state.phi[i, mw], ps_state.phi[j, mw] = (
            ps_state.phi[j, mw].copy(),
            ps_state.phi[i, mw].copy(),
        )
        ps_state.m[i, mw], ps_state.m[j, mw] = (
            ps_state.m[j, mw].copy(),
            ps_state.m[i, mw].copy(),
        )
        if ps_state.rest is not None:
            ps_state.rest[i, mw], ps_state.rest[j, mw] = (
                ps_state.rest[j, mw].copy(),
                ps_state.rest[i, mw].copy(),
            )
        ps_state.logpi[i, mw], ps_state.logpi[j, mw] = (
            ps_state.logpi[j, mw].copy(),
            ps_state.logpi[i, mw].copy(),
        )
...
def compute_bd_hazards_all(
    ...
    *,
    ...
    if bd_rate_scale > 0.0:
        log_base = np.log(bd_rate_scale) + np.log(
            np.maximum(betas[:, None], 1e-300)
        )  # (C,1)
    else:
    ...
    lam_total = np.where(
        np.isfinite(log_lam_total), np.exp(log_lam_total), 0.0
    )  # (C,W)
...
def restrict_slot_np(
    prop_all: np.ndarray, cur_all: np.ndarray, slot_slice_or_mask
) -> np.ndarray:
    out = cur_all.copy()
...
def _scatter_into(
    full: np.ndarray, idx: tuple[np.ndarray, np.ndarray], vals: np.ndarray
) -> np.ndarray:
...
def gibbs_mh_sweep_active_np(
    ...
    *,
    ...
    def _slot_prior_subset(
        phi_slot_full: np.ndarray, idx: tuple[np.ndarray, np.ndarray]
    ) -> np.ndarray:
    ...
    for j in range(Kmax):  ### loop over all sources
        ...
        if do_stretch:
            ...
            if maxdiff > 1e-6:  # choose your tolerance
                print(
                    f"[warn] LL mismatch after STRETCH slot {j}: max |Δ|={maxdiff:.3e}"
                )
    ...
    if do_PTswap:
        ...
        # even pass
        pt_state, acc_e, att_e = pt_swap_pass_numpy(
            rng, pt_state, betas, even_pass=True
        )
        ps_swap_pass_inplace(ps_state, acc_e, even_pass=True)
        # odd pass
        pt_state, acc_o, att_o = pt_swap_pass_numpy(
            rng, pt_state, betas, even_pass=False
        )
